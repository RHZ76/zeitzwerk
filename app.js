...
$('#backup').onclick=()=>download(`zeitzwerk-backup-${today()}.json`,JSON.stringify({app:'Zeit(z)Werk',version:'1.1.3',exported:new Date().toISOString(),entries},null,2),'application/json');
...
function printFiltered(openOnly){
 ...
 const cleanup=()=>{document.title=oldTitle;sheet.remove();window.removeEventListener('afterprint',cleanup)};
